...
export class SearchPage extends Page {
  ...
  pageContent() {
    ...
    const hasNoResults = this.state.validQuery && this.state.resultsCount === 0;
    const searchTitle = this.state.resultsTitle || translateString('Search media');

    return (
      <>
        <section className="discovery-page-intro">
          <span className="discovery-page-kicker">{translateString('DISCOVER')}</span>
          <h1>{searchTitle}</h1>
          <p>{translateString('Use filters to narrow results by media type, upload date and popularity.')}</p>
        </section>

        <MediaListWrapper className="search-results-wrap items-list-hor">
          ...
          {!this.state.validQuery ? (
            <div className="discovery-empty-state">
              ...
              <h2>{translateString('Search the media library')}</h2>
              <p>{translateString('Enter a keyword in the search box above to get started.')}</p>
            </div>
          ) : (
            <>
              ...
              {hasNoResults ? (
                <div className="discovery-empty-state">
                  ...
                  <h2>{translateString('No matching media')}</h2>
                  <p>{translateString('Try a shorter keyword or remove one of the filters.')}</p>
                </div>
              ) : null}
            </>
          )}
