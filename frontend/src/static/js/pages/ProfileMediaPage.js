        ...
        return [
            ...
            this.state.author ? (
                <ProfilePagesContent key="ProfilePagesContent">
                    {isMediaAuthor && !inEmbeddedApp() ? (
                        <LinksConsumer>
                            {(links) => (
                                <nav className="user-center-shortcuts" aria-label={translateString('User center')}>
                                    <a href={links.user.addMedia}>
                                        <span className="material-icons" aria-hidden="true">upload</span>
                                        <strong>{translateString('Upload media')}</strong>
                                        <small>{translateString('Add a new video or media file')}</small>
                                    </a>
                                    <a href={links.manage.media}>
                                        <span className="material-icons" aria-hidden="true">monitor_heart</span>
                                        <strong>{translateString('Upload records')}</strong>
                                        <small>{translateString('Review processing and publishing status')}</small>
                                    </a>
                                    <a href={links.profile.playlists}>
                                        <span className="material-icons" aria-hidden="true">playlist_play</span>
                                        <strong>{translateString('My playlists')}</strong>
                                        <small>{translateString('Create and organize saved media')}</small>
                                    </a>
                                    <a href={links.user.editProfile}>
                                        <span className="material-icons" aria-hidden="true">manage_accounts</span>
                                        <strong>{translateString('Edit profile')}</strong>
                                        <small>{translateString('Update personal and channel information')}</small>
                                    </a>
                                </nav>
                            )}
                        </LinksConsumer>
