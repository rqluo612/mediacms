...
import { apiFetch, translateString } from '../utils/helpers/';
...
export const BulkActionPlaylistModal: React.FC<BulkActionPlaylistModalProps> = ({
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const playlistsResponse = await apiFetch(`/api/v1/playlists?author=${encodeURIComponent(username)}`);
      ...
      const membershipResponse = await apiFetch('/api/v1/media/user/bulk_actions', {
        method: 'POST',
        headers: {
  ...
  const handleCreatePlaylist = async () => {
    ...
    try {
      const response = await apiFetch('/api/v1/playlists', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRFToken': csrfToken,
    ...
    try {
      ...
      if (toAdd.length > 0) {
        ...
        if (playlistIds.length > 0) {
          const addResponse = await apiFetch('/api/v1/media/user/bulk_actions', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'X-CSRFToken': csrfToken,
      ...
      if (toRemove.length > 0) {
        ...
        if (playlistIds.length > 0) {
          const removeResponse = await apiFetch('/api/v1/media/user/bulk_actions', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'X-CSRFToken': csrfToken,
