...
export function TaxonomyItemMediaCount(props) {
    // Older/dev configurations may not define `features.listings`.
    // Only hide counts when the setting is explicitly disabled.
    if (window.MediaCMS?.features?.listings?.includeNumbers === false) {
        return null;
    }
