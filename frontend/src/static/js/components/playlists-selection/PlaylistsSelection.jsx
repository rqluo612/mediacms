...
function PlaylistsSingleSelection(props) {
  function onChange(ev) {
    ev.persist();
    props.onPending(props.playlistId);
  ...
  return !!props.renderDate ? (
    <label>
      <input type="checkbox" checked={props.isChecked} disabled={props.isPending} onChange={onChange} />
      <span>{props.title}</span>
      {props.isPending ? <span className="playlist-action-pending">Saving…</span> : null}
...
export function PlaylistsSelection(props) {
      ...
      ret.push(
        <div key={'playlist_' + playlists[i].playlist_id}>
          <PlaylistsSingleSelection
            ...
            isPending={pendingPlaylistId === playlists[i].playlist_id}
            onPending={setPendingPlaylistId}
          />
        </div>
      );
      i += 1;
  ...
  return (
    <div ref={containerRef} className="saveto-popup">
      ...
      ) : openCreatePlaylist ? null : (
        <p className="saveto-empty">You have no playlists yet. Create one below.</p>
      )}
      {openCreatePlaylist ? (
